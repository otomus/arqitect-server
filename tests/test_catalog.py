"""Catalog tests: discover_nerves returns all nerves on disk, senses always present."""

import os
from unittest.mock import patch

import pytest
from dirty_equals import IsStr
from hypothesis import given, settings, strategies as st, HealthCheck

from arqitect.types import Sense

CORE_SENSES = tuple(Sense)

# Module-level store -- each test writes its catalog here, autouse fixture checks it.
_last_catalog = {}


@pytest.fixture(autouse=True)
def assert_enabled_senses_in_catalog():
    """After every test, verify enabled core senses are present in the catalog."""
    _last_catalog.clear()
    yield
    if _last_catalog.get("data") is not None:
        catalog = _last_catalog["data"]
        for sense in CORE_SENSES:
            if _last_catalog.get(f"disabled_{sense}"):
                assert sense not in catalog, \
                    f"Disabled sense '{sense}' should NOT be in catalog"
            else:
                assert sense in catalog, \
                    f"Core sense '{sense}' missing from catalog: {list(catalog.keys())}"
                assert catalog[sense], f"Sense '{sense}' has empty description"


def _make_nerve_file(nerves_dir, name):
    """Create a minimal nerve.py on disk."""
    d = os.path.join(nerves_dir, name)
    os.makedirs(d, exist_ok=True)
    with open(os.path.join(d, "nerve.py"), "w") as f:
        f.write("# stub\n")


@pytest.mark.timeout(10)
class TestDiscoverNervesIncludesAll:
    """All nerves on disk should be discoverable."""

    def test_nerve_on_disk_included(self, nerves_dir, mem):
        """Any nerve on disk should appear in discover_nerves."""
        _make_nerve_file(nerves_dir, "good_nerve")

        with patch("arqitect.brain.catalog.mem", mem):
            from arqitect.brain.catalog import discover_nerves
            catalog = discover_nerves()
            _last_catalog["data"] = catalog
            assert "good_nerve" in catalog

    def test_nerve_on_disk_without_registry(self, nerves_dir, mem):
        """discover_nerves finds nerves from filesystem even without cold memory registration."""
        _make_nerve_file(nerves_dir, "new_nerve")

        with patch("arqitect.brain.catalog.mem", mem):
            from arqitect.brain.catalog import discover_nerves
            catalog = discover_nerves()
            _last_catalog["data"] = catalog
            assert "new_nerve" in catalog

    def test_zero_score_nerve_on_disk_still_included(self, nerves_dir, mem):
        """Even a nerve that failed qualification is discovered if on disk."""
        _make_nerve_file(nerves_dir, "failed_nerve")
        mem.cold.register_nerve("failed_nerve", "always fails")
        mem.cold.record_qualification(
            "nerve", "failed_nerve", qualified=False, score=0.0,
            iterations=5, test_count=15, pass_count=0,
        )

        with patch("arqitect.brain.catalog.mem", mem):
            from arqitect.brain.catalog import discover_nerves
            catalog = discover_nerves()
            _last_catalog["data"] = catalog
            assert "failed_nerve" in catalog

    @given(
        nerve_name=st.from_regex(r"[a-z][a-z0-9_]{2,15}_nerve", fullmatch=True),
    )
    @settings(max_examples=10, suppress_health_check=[HealthCheck.function_scoped_fixture])
    def test_any_valid_nerve_discovered(self, nerves_dir, mem, nerve_name):
        """Any validly named nerve with nerve.py on disk gets discovered."""
        _make_nerve_file(nerves_dir, nerve_name)

        with patch("arqitect.brain.catalog.mem", mem):
            from arqitect.brain.catalog import discover_nerves
            catalog = discover_nerves()
            _last_catalog["data"] = catalog
            assert nerve_name in catalog


@pytest.mark.timeout(10)
class TestSensesAlwaysPresent:
    """Core senses must always appear in the catalog."""

    def test_all_five_senses_in_catalog(self, nerves_dir, mem):
        """All 5 core senses must appear regardless of qualification state."""
        with patch("arqitect.brain.catalog.mem", mem):
            from arqitect.brain.catalog import discover_nerves
            catalog = discover_nerves()
            _last_catalog["data"] = catalog

    def test_senses_have_descriptions(self, nerves_dir, mem):
        """Each sense should have a non-empty description."""
        with patch("arqitect.brain.catalog.mem", mem):
            from arqitect.brain.catalog import discover_nerves
            catalog = discover_nerves()
            _last_catalog["data"] = catalog
            for sense in CORE_SENSES:
                assert catalog[sense] == IsStr(min_length=1)


@pytest.mark.timeout(10)
class TestDisabledSenses:
    """Disabled senses should be excluded from the catalog."""

    def test_disabled_sense_excluded_from_catalog(self, nerves_dir, mem):
        """A sense with enabled: false in config should not appear."""
        with patch("arqitect.brain.catalog.mem", mem), \
             patch("arqitect.brain.catalog.get_config") as mock_cfg:

            def _cfg(path, default=None):
                if path == "senses.hearing.enabled":
                    return False
                return default if default is not None else True

            mock_cfg.side_effect = _cfg

            from arqitect.brain.catalog import discover_nerves
            catalog = discover_nerves()
            _last_catalog["data"] = catalog
            _last_catalog["disabled_hearing"] = True
            assert "hearing" not in catalog

    def test_all_senses_disabled(self, nerves_dir, mem):
        """When all senses are disabled, none appear in catalog."""
        with patch("arqitect.brain.catalog.mem", mem), \
             patch("arqitect.brain.catalog.get_config", return_value=False):
            from arqitect.brain.catalog import discover_nerves
            catalog = discover_nerves()
            for sense in CORE_SENSES:
                _last_catalog[f"disabled_{sense}"] = True
            _last_catalog["data"] = catalog
            for sense in CORE_SENSES:
                assert sense not in catalog


@pytest.mark.timeout(10)
class TestFilesystemFallback:
    """Filesystem-only nerves get auto-registered."""

    def test_nerve_on_disk_but_not_in_registry_gets_registered(self, nerves_dir, mem):
        """A nerve that exists on disk but not in cold memory should be auto-registered."""
        _make_nerve_file(nerves_dir, "orphan_nerve")

        with patch("arqitect.brain.catalog.mem", mem):
            from arqitect.brain.catalog import discover_nerves
            catalog = discover_nerves()
            _last_catalog["data"] = catalog
            all_nerves = mem.cold.list_nerves()
            assert "orphan_nerve" in all_nerves, \
                f"Orphan nerve should be auto-registered. Registry: {list(all_nerves.keys())}"
