...
_INTENT_SYSTEM = (
    ...
    "1. \"workflow\" — the user wants a multi-step PROCESS: development, debugging, "
    "project setup, planning, migration, deployment, refactoring, investigation. "
    "These imply structured work with multiple coordinated steps.\n"
    "2. \"direct\" — everything else: greetings, questions, single requests, "
    "creative tasks, lookups, conversations, simple commands.\n\n"
    "If the type is \"workflow\", also include a \"category\" field if obvious "
    "(e.g. \"development\", \"debugging\", \"setup\", \"planning\", \"migration\", "
    "\"deployment\", \"refactoring\").\n\n"
    "Reply with ONLY a JSON object. Examples:\n"
    '{"type": "direct"}\n'
    '{"type": "workflow", "category": "development"}\n'
    '{"type": "workflow", "category": "debugging"}\n'
    '{"type": "direct"}\n'
)


def classify_intent(task: str) -> dict:
    """Classify a user message as workflow or direct.

    Returns:
        {"type": "workflow", "category": "..."} or {"type": "direct"}
    """
    prompt = f"Classify this message:\n\n{task}"
    raw = llm_generate(BRAIN_MODEL, prompt, system=_INTENT_SYSTEM)

    result = extract_json(raw)
    if result and result.get("type") in (IntentType.WORKFLOW, IntentType.DIRECT):
        return result
